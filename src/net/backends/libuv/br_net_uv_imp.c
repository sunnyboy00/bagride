...
void br_tcp_register(br_tcp_server_t* tcp_servers_, int size_){
    ...
    for (i = 0; i < size_; i++) {
        ...
        MM_ASSERT(0 == uv_ip4_addr("0.0.0.0", server->m_port, &server->m_socket));
        MM_ASSERT(0 == uv_tcp_bind(&server->m_handler, (const struct sockaddr*) &server->m_socket));
...
void br_udp_register(br_udp_server_t* udp_servers_, int size_) {
    ...
    for (i = 0; i < size_; i++) {
        ...
        server->m_handler.data = server;
        MM_ASSERT(0 == uv_ip4_addr("0.0.0.0", server->m_port, &server->m_socket));
        MM_ASSERT(0 == uv_udp_bind(&server->m_handler, (const struct sockaddr*) &server->m_socket, 0));
