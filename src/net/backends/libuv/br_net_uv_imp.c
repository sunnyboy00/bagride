...
static void on_tcp_read(uv_stream_t* stream_, ssize_t nread_, const uv_buf_t* read_buf_) {
    if (nread_ > 0) {
        ...
        R_ASSERT(0 == user_parse_cb(nread_, read_buf_, tcp_server));
        if (write_buffer->len) {
...
static void on_udp_recv(uv_udp_t* stream_, ssize_t nread_, const uv_buf_t* read_buff_,
    ...
    if (nread_ > 0) {
        ...
        R_ASSERT(0 == user_parse_cb(nread_, read_buff_, server_udp));
    }
...
void br_tcp_register(br_tcp_server_t* tcp_servers_, int size_){
    ...
    for (i = 0; i < size_; i++) {
        ...
        R_INFO("(%d) tcp listening on port %d", i, server->m_port);
        uv_tcp_init(uv_default_loop(), &server->m_handler);
        server->m_handler.data = server;
        R_ASSERT(0 == uv_ip4_addr("0.0.0.0", server->m_port, &server->m_socket));
        R_ASSERT(0 == uv_tcp_bind(&server->m_handler, (const struct sockaddr*) &server->m_socket));
        R_ASSERT(0 == uv_listen((uv_stream_t*) & server->m_handler, BR_MAX_CONNECTIONS, on_connect));
    }
}
...
void br_udp_register(br_udp_server_t* udp_servers_, int size_) {
    ...
    for (i = 0; i < size_; i++) {
        ...
        R_INFO("(%d) udp listening on port %d", i, server->m_port);
        uv_udp_init(uv_default_loop(), &server->m_handler);
        server->m_handler.data = server;
        R_ASSERT(0 == uv_ip4_addr("0.0.0.0", server->m_port, &server->m_socket));
        R_ASSERT(0 == uv_udp_bind(&server->m_handler, (const struct sockaddr*) &server->m_socket, 0));
        R_ASSERT(0 == uv_udp_recv_start(&server->m_handler, on_alloc_buffer, on_udp_recv));
    }
}
/**
 * http
...
static void on_http_close(uv_handle_t* handle_) {
    ...
    R_INFO("(%5d) connection closed ", cli->m_request_num);
...
static void on_http_connect(uv_stream_t* handle_, int status_) {
    ...
    R_INFO("(%5d) connection new %p", cli->m_request_num, handle_);
...
void br_http_register(br_http_server_t* servers_, int size_) {
    ...
    for (i = 0; i < size_; i++) {
        ...
        R_INFO("(%d) http listening on port %d", i, server->m_port);
        uv_tcp_init(uv_default_loop(), &server->m_handler);
        server->m_handler.data = server;
        R_ASSERT(0 == uv_ip4_addr("0.0.0.0", server->m_port, &server->m_addr));
        R_ASSERT(0 == uv_tcp_bind(&server->m_handler, (const struct sockaddr*) &server->m_addr));
        R_ASSERT(0 == uv_listen((uv_stream_t*) & server->m_handler, BR_MAX_CONNECTIONS, on_http_connect));
    }
}
